...
type SubscribableEvent = "turn_end" | "message_start" | "message_end" | "tool_execution_start" | "tool_execution_end";
...
async function handleCommand(
	...
	if (command.type === "subscribe") {
		const validEvents: SubscribableEvent[] = [
			"turn_end",
			"message_start",
			"message_end",
		...
		if (!validEvents.includes(command.event as SubscribableEvent)) {
			respond(false, "subscribe", undefined, `Unknown event type: ${command.event}. Valid: ${validEvents.join(", ")}`);
			return;
		}

		const eventName = command.event as SubscribableEvent;
		const subscriptionId = id ?? `sub_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
		const subscription: EventSubscription = { socket, subscriptionId };

		// Legacy path for turn_end (one-shot).
		if (eventName === "turn_end") {
			state.turnEndSubscriptions.push(subscription);
		} else {
			let subs = state.eventSubscriptions.get(eventName);
			if (!subs) {
				subs = [];
				state.eventSubscriptions.set(eventName, subs);
			}
		...
		const cleanup = () => {
			if (eventName === "turn_end") {
				...
				const subs = state.eventSubscriptions.get(eventName);
				if (subs) {
		...
		respond(true, "subscribe", { subscriptionId, event: eventName });
		return;
	}
	...
	if (command.type === "reload") {
		if (!ctx.isIdle()) {
			respond(false, "reload", undefined, "Session is busy - wait for turn to complete");
			return;
		}
		// Queue a /reload-runtime command as a follow-up message.
		pi.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
		respond(true, "reload", { queued: true });
		return;
	}
...
export default function (pi: ExtensionAPI) {
	...
	// Command entrypoint for ctx.reload() (only ExtensionCommandContext has reload).
	...
	// Broadcast to persistent event subscribers (message/tool lifecycle events).
	function broadcastEvent(eventName: SubscribableEvent, data: unknown): void {
		const subs = state.eventSubscriptions.get(eventName);
